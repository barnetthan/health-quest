...
function QuestPage() {
  ...
  const handleDeleteDefaultGoal = async (field: "healthyFats" | "veggies" | "cardio" | "strength") => {
    if (!currentUser || !group) return;
  ...
  const calculateTotalProgress = () => {
    ...
    // Custom goals
    const customTotal = customGoals.reduce((acc, goal) => acc + (goal.current || 0), 0);
    const customMaxTotal = customGoals.reduce((acc, goal) => acc + goal.target, 0);
          ...
          <div key={goal.id} className="card mb-3 p-2">
            <div className="d-flex justify-content-between mb-2">
              <div className="d-flex align-items-center">
                ...
                <b>{goal.name}: {goal.target} Servings</b>
              </div>
              <div className="d-flex align-items-center">
          ...
          {customGoals.map((goal) => (
            <div key={goal.id} className="list-group-item">
              <div className="d-flex justify-content-between align-items-center">
                <div className="d-flex align-items-center">
                  <span className={`me-2 text-${goal.category === 'nutrition' ? 'success' : 'primary'}`}>
          ...
          <Form>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select
                ...
                onChange={(e) => setNewGoal({...newGoal, category: e.target.value as 'nutrition' | 'fitness'})}
              >
